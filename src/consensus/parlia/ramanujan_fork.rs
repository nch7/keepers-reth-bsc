...
use crate::consensus::parlia::{
    Snapshot, FIXED_BACKOFF_TIME_BEFORE_FORK_MILLIS, MILLISECONDS_UNIT,
    WIGGLE_TIME_BEFORE_FORK_MILLIS,
};
    ...
    ChainSpec: EthChainSpec + BscHardforks + 'static,
{
    ...
    pub fn block_time_for_ramanujan_fork(
        &self,
        snap: &Snapshot,
        parent: &Header,
        header: &Header,
    ...
    pub fn delay_for_ramanujan_fork(&self, parent_snap: &Snapshot, header: &Header) -> u64 {
        ...
        if header.difficulty == DIFF_NOTURN {
            let wiggle =
                (parent_snap.validators.len() / 2 + 1) as u64 * WIGGLE_TIME_BEFORE_FORK_MILLIS;
    ...
    pub fn block_time_verify_for_ramanujan_fork(
        &self,
        snap: &Snapshot,
        header: &Header,
        parent: &Header,
