...
use crate::consensus::parlia::util::{debug_header, is_breathe_block};
use crate::consensus::parlia::vote::MAX_ATTESTATION_EXTRA_LENGTH;
use crate::consensus::parlia::{Snapshot, VoteAddress, DIFF_INTURN, DIFF_NOTURN};
...
use reth_evm::{
    eth::receipt_builder::ReceiptBuilder, execute::BlockExecutionError, Database, Evm,
...
pub static VALIDATOR_CACHE: LazyLock<Mutex<ValidatorCache>> =
    LazyLock::new(|| Mutex::new(LruMap::new(ByLength::new(1024))));

pub static TURN_LENGTH_CACHE: LazyLock<Mutex<TurnLengthCache>> =
    LazyLock::new(|| Mutex::new(LruMap::new(ByLength::new(1024))));
    ...
    R::Transaction: Into<TransactionSigned>,
{
    ...
    pub(crate) fn check_new_block(&mut self, block: &BlockEnv) -> Result<(), BlockExecutionError> {
        ...
        if header.number.is_multiple_of(epoch_length) {
            ...
            let (validator_set, vote_addresses) =
                self.get_current_validators_with_cache(header.number - 1, header.parent_hash)?;
            tracing::debug!(
                "validator_set: {:?}, vote_addresses: {:?}",
                validator_set,
                vote_addresses
            );
            ...
            if self.spec.is_maxwell_active_at_timestamp(header.number, header.timestamp) {
                let (to2, data2) = self
                    .system_contracts
                    .get_node_ids(self.inner_ctx.current_validators.as_ref().unwrap().0.clone());
                if let Ok(output2) = self.eth_call(to2, data2) {
                    let (_consensus_addrs, node_ids_list) =
                        self.system_contracts.unpack_data_into_node_ids(&output2);
                    tracing::debug!("node_ids_list: {:?}", node_ids_list);
                    let mut flat: Vec<[u8; 32]> = Vec::new();
                    for ids in node_ids_list {
                        for id in ids {
                            flat.push(id);
                        }
                    }
            ...
            && is_breathe_block(parent_header.timestamp, header.timestamp)
        {
            ...
            let max_elected_validators =
                self.system_contracts.unpack_data_into_max_elected_validators(bz.as_ref());
    ...
    pub(crate) fn eth_call(
        ...
        let output = result_and_state
            .result
            .output()
            .ok_or(BlockExecutionError::msg("ETH call output is None"))?;
        Ok(output.clone())
    }
    ...
    pub(crate) fn get_current_validators(
        ...
        let result = if self.spec.is_luban_active_at_block(block_number) {
            ...
            let output = self.eth_call(to, data)?;
            let validator_set =
                self.system_contracts.unpack_data_into_validator_set_before_luban(&output);
    ...
    pub(crate) fn get_justified_header(
        ...
        if snap.vote_data.source_hash == B256::ZERO && snap.vote_data.target_hash == B256::ZERO {
            return HEADER_CACHE_READER.lock().unwrap().get_header_by_number(0).ok_or_else(|| {
